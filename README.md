...
auth.users（Supabase管理）
  └── users
        ...
        │     └── exchange_requests（offer_clip_id / want_clip_id）
...
| is_public | BOOLEAN | NOT NULL, DEFAULT true | ブック公開/非公開 |
...
>
> 交換成立時はuser_idが更新される。page/positionは相手ブックの末尾に再設定される。
...
|---|---|---|---|
| id | UUID | PK | リクエストID |
| from_user_id | UUID | NOT NULL, FK→users | リクエスト送信者 |
| to_user_id | UUID | NOT NULL, FK→users | リクエスト受信者 |
| offer_clip_id | UUID | NOT NULL, FK→clips | 自分が差し出すカード |
| want_clip_id | UUID | NOT NULL, FK→clips | 相手から欲しいカード |
...
| 2 | DBマイグレーション | Supabaseで全テーブル作成（5テーブル） |
...
| Phase | 内容 |
|---|---|
| Phase 2-1 | ドラッグ＆ドロップ並び替え（dnd-kit + PATCH /clips/reorder） |
| Phase 2-2 | 他ユーザーのスクラップブック閲覧（GET /clips?user_id=xxx） |
| Phase 2-3 | 公開・非公開設定（PATCH /clips/{id} の is_public） |
| Phase 2-4 | 手動トリミングUI（Cropper.js） |
| Phase 2-5 | ログイン時の表紙めくりアニメーション |
